...
parser.add_argument("-w", "--workingdir", dest="working_dir",
                    help="The working directory of this script (default: script location)",
                    type=dir_path, required=True)
parser.add_argument("-l", "--lexicaldata", action="append", dest="datasets",
                    help="Space seperated list of datasets containing labels "
                         + "and synonyms and their corresponding GECKO term.")
...
df_simplestring_out.drop_duplicates().to_csv(args.zooma_dataset, index=False, sep="\t",
